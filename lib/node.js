...
export class NODE {
  constructor() {
    ...
    this.postStartModules = ['swarm', 'messages']
  }
  ...
  async init() {
    ...
    //Loading the post startup modules that need the node db (waiting for each module to stop before starting the next)
    for await (const module of this.postStartModules) {
      await this.addModule(module)
    }

    //Fill the database
    //With IPFS Id
    await this.db.database.put('ipfsId', this.ipfs.id)
    ...
    //Put the messages database id
    await this.db.database.put(
      'messagesDbId',
      this.messages.db.database.address.root
    )
