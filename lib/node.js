...
export class Node {
  ...
  async init() {
    ...
    if (!this.getName()) {
      if (isNode) {
        ...
        await this.setName('John DOE')
        await this.setFirstName('John')
        await this.setLastName('DOE')
      }
    }
    ...
    this.ipfs.ipfs.libp2p.on('peer:connect', async peer => {
      await this.swarm.setOnline(peer.id._idB58String)
      await this.db.database.put('peers', this.ipfs.getSwarmPeers())
    ...
    this.ipfs.ipfs.libp2p.on('peer:disconnect', async peer => {
      await this.swarm.setOffline(peer.id._idB58String)
      await this.db.database.put('peers', this.ipfs.getSwarmPeers())
  ...
  //Set the node name
  async setName(name) {
    var name = name ? name : this.getFirstName() + ' ' + this.getLastName()
