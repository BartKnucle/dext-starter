...
export class NODE extends MODULE {
  ...
  async register() {
    ...
    swarmData.type = this.get('plateform.type').data

    if (isNode) {
      swarmData.name = this.get('plateform.hostname').data
    } else {
      swarmData.name = this.get('user.fullname').data
    }

    swarmData.alive = true

    //Add the node in the database
    await this.app.swarm.add({
      id: this.dbId,
      data: swarmData
    })
